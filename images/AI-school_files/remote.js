...
break;case -1:this.Pk(new g.EH(64));break;case -1E3:this.Pk(new g.EH(128,{errorCode:"mdx.remoteerror",errorMessage:"Cette vid\u00e9o ne peut pas \u00eatre visionn\u00e9e \u00e0 distance.",nQ:2}))}w=s9(this.Hj).trackData;var M=this.X;(w||M?w&&M&&w.trackName==M.trackName&&w.languageCode==M.languageCode&&w.languageName==M.languageName&&w.kind==M.kind:1)||(this.X=w,kJj(this,w));w=s9(this.Hj);w.volume===-1||Math.round(this.U.getVolume())===w.volume&&this.U.isMuted()===w.muted||this.S.isActive()||this.Dd()}else BGA(this)};
...
S$.prototype.g7=function(w){if(this.api.getPresentingPlayerType()===3){var M={RECEIVER_NAME:this.api.getOption("remote","currentReceiver").name};w=g.C(w,128)?g.iM("Erreur sur $RECEIVER_NAME",M):w.isPlaying()||w.isPaused()?g.iM("Lecture sur $RECEIVER_NAME",M):g.iM("Connect\u00e9 \u00e0 $RECEIVER_NAME",M);this.updateValue("statustext",w);this.fade.show()}else this.fade.hide()};g.V(HB,g.v6);HB.prototype.X=function(){var w=this.U.getOption("remote","receivers");w&&w.length>1&&!this.U.getOption("remote","quickCast")?(this.g0=g.xV(w,this.B,this),this.cI(g.EV(w,this.B)),w=this.U.getOption("remote","currentReceiver"),w=this.B(w),this.options[w]&&this.JV(w),this.enable(!0)):this.enable(!1)};
HB.prototype.B=function(w){return w.key};
HB.prototype.SH=function(w){return w==="cast-selector-receiver"?"Caster sur\u2026":this.g0[w].name};
